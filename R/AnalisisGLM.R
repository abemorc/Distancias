...
dir.create("Analisis estadistico")
ruta <- getwd()
dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs"))
dir.create(file.path(ruta, "Analisis estadistico", "Distancia total"))

dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Tablas"))
dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Graficos"))

dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Tablas", "Correlacion"))
dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Graficos", "Correlacion"))

dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Tablas", "Correlacion", "Cuantitativas"))
dir.create(file.path(ruta, "Analisis estadistico", "Distancia24hrs", "Tablas", "Correlacion", "Cualitativas"))


dir.create("Graficas")
dir.create("Tablas")

rutaGrap <- file.path(ruta, "Graficas")
rutaTabl <- file.path(ruta, "Tablas")

dir.create(path = file.path(rutaGrap, "Analisis correlacion"))
dir.create(path = file.path(rutaTabl, "Analisis correlacion"))
rutaGrapCorr <- file.path(rutaGrap, "Analisis correlacion")
rutaTablCorr <- file.path(rutaTabl, "Analisis correlacion")
...
str(dfdatos1)
View(dfdatos1)
ldf[[length(ldf)+1]] <- dfdatos1
vnombresDat <- c(vnombresDat, "1 Datos con variables dummy")


nombre <- "1 Datos con variables dummy.csv"
write.csv(x = dfdatos1, file = file.path(rutaTabl, nombre))
...
grap <- recordPlot()
lgrap[[length(lgrap)+1]] <- grap
vnombresGrap <- c(vnombresGrap, "1 Histograma distribucion Distancia24hrs")


nombre <- "1 Histograma distribucion Distancia24hrs"

nombre <- paste0(rutaGrap, "/", nombre, ".png")
png(file = nombre)
  hist(x = dfdatos1$Distancia24hrs, breaks = 7, xlab = "Distancia recorrida", main = "Distribucion datos 24hrs")
dev.off()
...
ldf[[length(ldf)+1]] <- dfdatos1Est
vnombresDat <- c(vnombresDat, "2 Datos estandarizados")


nombre <- "2 Datos estandarizados.csv"
write.csv(x = dfdatos1Est, file = file.path(rutaTabl, nombre))
...
ldf[[length(ldf)+1]] <- correlaciones
vnombresDat <- c(vnombresDat, "3 Matriz de correlaciones spearman")


nombre <- "3 Matriz de correlaciones Spearman.csv"
write.csv(x = correlaciones, file = file.path(rutaTabl, nombre))
...
ldf[[length(ldf)+1]] <- correlaciones1
vnombresDat <- c(vnombresDat, "4 Matriz de correlaciones spearman sin estd")


nombre <- "4 Matriz de correlaciones Spearman estandarizados.csv"
write.csv(x = correlaciones1, file = file.path(rutaTabl, nombre))
...
ldf[[length(ldf)+1]] <- corrpearson
vnombresDat <- c(vnombresDat, "5 Matriz de correlaciones pearson")

nombre <- "5 Matriz de correlaciones Pearson.csv"
write.csv(x = corrpearson, file = file.path(rutaTabl, nombre))

#pearson sobre datos originales
corrpearson1 <- cor(dfdatos1[c(1:8)], use ="complete.obs", method = "pearson")
corrpearson1

ldf[[length(ldf)+1]] <- corrpearson1
vnombresDat <- c(vnombresDat, "6 Matriz de correlaciones pearson estd")

nombre <- "6 Matriz de correlaciones Pearson estandarizados.csv"
write.csv(x = corrpearson1, file = file.path(rutaTabl, nombre))
...
ldf[[length(ldf)+1]] <- probcor$P
vnombresDat <- c(vnombresDat, "7 Matriz significancia de las correlaciones")


nombre <- "7 Matriz significancia de las correlaciones.csv"
write.csv(x = probcor$P, file = file.path(rutaTabl, nombre))
...
ldf[[length(ldf)+1]] <- rel
vnombresDat <- c(vnombresDat, "8 Importancia variables cuantitativas sobre D24hrs")
...
ldf[[length(ldf)+1]] <- probcor1
vnombresDat <- c(vnombresDat, "9 Significancia variables cuantitativas sobre D24hrs")
...
grap <- recordPlot()
lgrap[[length(lgrap)+1]] <- grap
vnombresGrap <- c(vnombresGrap, "2 Correlaciones spearman")

#chart.Correlation(dfdatos1Est[c(1:8)], histogram = T,
#                  method = "spearman", exact = FALSE, stars = TRUE)


pairs(dfdatos1Est[c(1:8)])
grap <- recordPlot()
lgrap[[length(lgrap)+1]] <- grap
vnombresGrap <- c(vnombresGrap, "3 correlaciones 1 a 1")
...
grap <- recordPlot()
lgrap[[length(lgrap)+1]] <- grap
vnombresGrap <- c(vnombresGrap, "4 Correlaciones completa")



#grafica1 <- recordPlot()
#save_plot(filename = "hola.png")
...
boxplot(formula = as.numeric(Distancia24hrs) ~ Sitio, data = na.omit(dfdatosori),
  ...
  grap <- recordPlot()
  lgrap[[length(lgrap)+1]] <- grap
  vnombresGrap <- c(vnombresGrap, "5 Boxplot sitio")
...
relacionSexo <- cor.test(dfdatos1Est$Sexo_macho, dfdatos1Est$Distancia24hrs)
relacionSexo <- cor.test(dfdatos1Est$Sex_hembra, dfdatos1Est$Distancia24hrs) #esto es solo para probar que la relacion es la misma pero inversamente
